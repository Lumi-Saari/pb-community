...
app.post('/:roomId/posts', async (c) => {
  ...
  where: { roomId },
  select: { roomName: true, islocked: true },
 });

 if (!room) return c.text('このルームは存在しません', 404);
 if(room.islocked && !existingUser.isAdmin) {
	return c.text('このルームはロックされています', 403);
 }
...
app.post('/:roomId/replies', async (c) => {
  ...
  const parent = await prisma.RoomPost.findUnique({ where: { postId: parentId }, select: { isLocked: true } });
  if (!parent) return c.text('返信先の投稿が見つかりません', 404);
  
  if(parent.isLocked) {
    return c.text('この投稿はロックされています。返信できません', 403);
  }
...
app.post('/:roomId/posts/:postId/lock', async (c) => {
  const { postId } = c.req.param();
  const { user } = c.get('session') ?? {};
  if (!user) return c.redirect('/login');
...
app.post('/:roomId/posts/:postId/unlock', async (c) => {
  const { postId } = c.req.param();
