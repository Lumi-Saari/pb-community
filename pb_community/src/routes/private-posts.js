...
if (existingUser.isLimited && existingUser.LimitExpiresAt) {
 ...
 if(!private) return c.text('このプライベートルームは存在しません', 404);
  if (private.isLocked) {
    return c.text('このプライベートルームはロックされています。投稿できません。', 403);
 }
...
app.post('/:privateId/replies', async (c) => {
  ...
  const existingUser = await prisma.user.findUnique({ where:  { userId: user.userId } });
  if(!existingUser) return c.redirect('/login')
...
app.delete('/:privateId/posts/:postId', async (c) => {
  ...
  const deleted = await prisma.PrivatePost.findUnique({
    where: { postId },
...
app.post('/:privateId/posts/:postId/lock', async (c) => {
  ...
  const isAdmin = user.isAdmin;
   ...
   const Limited = await prisma.PrivatePost.findUnique({
    where: { postId },
    select: { content: true, user: { select: { username: true } } }
   })
